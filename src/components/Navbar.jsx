...
function Navbar({ currentPage, setCurrentPage, isLoggedIn, handleLogin, handleLogout }) {
  const [darkMode, setDarkMode] = useState(false);
  ...
  return (
    <nav className="bg-gradient-to-r from-yellow-400 to-yellow-500 dark:from-gray-900 dark:to-gray-800 shadow-lg sticky top-0 z-50 transition-all">
      <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div className="space-x-6 flex items-center">
          ...
          <button
            onClick={() => isLoggedIn && setCurrentPage("verify")}
            disabled={!isLoggedIn}
            className={`font-semibold px-4 py-2 rounded-lg transition-all ${
              !isLoggedIn
          ...
          {!isLoggedIn ? (
            <button
              onClick={handleLogin}
              className="ml-4 px-4 py-2 rounded-lg bg-gray-900 text-white hover:bg-gray-800 dark:bg-white dark:text-gray-900 font-semibold transition"
            >
              Sign In / Sign Up
            </button>
          ) : (
            <button
              ...
              className="ml-4 px-4 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600 font-semibold transition"
            >
              Logout
            </button>
          )}
